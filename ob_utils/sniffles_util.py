...
def simplify_sniffles(in_control_bedpe, min_support_read, hout, h_chrom_number):
...
def snifflesSVtoBedpe_main(args):
    ...
    with open(output_prefix+'.sniffles_control_simplify.bedpe', 'w') as hout:
        simplify_sniffles(output_prefix+'.sniffles_control_PASS.bedpe', min_control_support_read, hout, h_chrom_number)
    ...
    filt_clustered_rearrangement2(output_prefix+'.sniffles_tumor_PASS.bedpe', output_prefix+'.sniffles_filtered.bedpe', 
    output_prefix+'.sniffles_control_sorted.bedpe.gz', margin, min_tumor_support_read, h_chrom_number)
