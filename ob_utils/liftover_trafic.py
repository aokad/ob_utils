...
def make_liftover_input(in_vcf, out_pref):
...
def write_result(in_vcf, out_vcf, h_chr, h_bkpb, h_src, h_tdc):
...
### Main ###
def liftover_trafic_main(args):
    
    out_pref, ext = os.path.splitext(args.out_vcf)
    
    make_liftover_input(args.in_vcf, out_pref)
    
    # liftover chromosome position
    subprocess.check_call(["liftOver", out_pref+ ".chr.bed", args.map_chain, out_pref+ ".chr_GRCh38.bed", out_pref+ ".chr_unmapped.bed"])
            
    # liftover bkpb
    subprocess.check_call(["liftOver", out_pref+ ".bkpb.bed", args.map_chain, out_pref+ ".bkpb_GRCh38.bed", out_pref+ ".bkpb_unmapped.bed"])
    
    # liftover src 
    subprocess.check_call(["liftOver", out_pref+ ".src.bed", args.map_chain, out_pref+ ".src_GRCh38.bed", out_pref+ ".src_unmapped.bed"])
