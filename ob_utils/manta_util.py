...
from .repair_bedpe import repair_dup_strand_cnv
...
def mantaCNVtoBedpe(input_vcf, output, f_grc, bcf_filter_option, filter_scaffold_option):

    out_pref, ext = os.path.splitext(output)

    if bcf_filter_option != "":
        subprocess.check_call(["bcftools", "view", "-e", "ALT='.'", "-o", out_pref + ".tmp1.vcf", "-f", bcf_filter_option, input_vcf])
    else:
        subprocess.check_call(["bcftools", "view", "-e", "ALT='.'", "-o", out_pref + ".tmp1.vcf", input_vcf])
    ...
    repair_dup_strand_cnv(out_pref + ".tmp2.bedpe", out_pref + ".tmp3.bedpe")
...
def mantaSVtoBedpe_main(args):
    ...
    if args.is_cnv:
        mantaCNVtoBedpe(args.in_manta_sv, args.output, args.f_grc, args.bcf_filter_option, args.filter_scaffold_option)
    else:
